...
**Status:** Fix implemented, verified in dev — not yet merged
...
**Verified:** `npm run build`/typecheck pass clean. Confirmed in dev via Playwright — `gtm.js?id=GTM-TBJDVSX4` request fires (200), `dataLayer` populates, and the `<noscript>` iframe is present in the DOM. No visible consent banner appeared in this dev check, but per the task's own scope that's expected — the banner's UI is controlled by the GTM container's own tag configuration, not this repo, so if it still doesn't appear after this ships, that's a GTM dashboard config question, not a code bug.
