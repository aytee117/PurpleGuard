...
## 1. Analytics + cookie consent banner in `next-app`

**Status:** Real, working implementation ported into `next-app` and verified locally via Playwright (consent banner, Consent Mode v2 gating, SPA pageview tracking, footer/privacy-page reopen controls all confirmed working end-to-end in dev). Not yet built/deployed — see outstanding steps below.

**Summary:** What looked like "the GTM cookie banner isn't showing on the new site" turned into a much bigger finding: `next-app` had **never** had a real, working analytics/consent install at all. The investigation (prompted by GA4 reporting "tag not detected in 48 hours" despite showing real historical traffic) worked backward through every layer — GTM's published container, GTM's draft workspace, this repo's entire git history on every branch, Cloudflare (Zaraz/Transform Rules/Workers), `next.config.js`/middleware/`vercel.json`, and GA4's Measurement Protocol API secrets — and found **nothing** in any of them. Every one was a dead end.

**Root cause:** the real implementation existed all along, just in a *different* repository — `aelhaddadmhe-alt/PurpleGuardRepl`, the original Replit-synced repo the site was actually built and live-served from. On **2026-04-23** (three days after the GA4 property/stream was created on 2026-04-20 — explaining exactly why GA4's real traffic history starts right around then), a complete implementation was built and published there: direct `gtag.js` (not GTM), Google Consent Mode v2 with defaults set `beforeInteractive`, SPA-aware pageview tracking, and a real Accept/Reject banner — and the leftover `GTM-TBJDVSX4` bootstrap script (installed back on 2026-01-28, and which — per its own GTM Version History — never had a single tag published in it, ever) was deliberately removed in that same work, not run alongside it. That work never made it into *this* repo (confirmed via `git log --all` here: these files have never existed in this repo, on any branch, ever) — this repo is an earlier snapshot that predates it. Once DNS was cut over from Replit to Vercel (which serves from this repo, not `PurpleGuardRepl`), real tracking silently stopped, with no trace here of anything having gone missing — hence the current Privacy page's own placeholder line, *"A cookie consent banner is being implemented"* (a leftover from this repo's earlier state, not a regression).

**Fix:** ported the working implementation from `PurpleGuardRepl` (cloned read-only into the session scratchpad per the user's instruction, never merged into this repo's git history) into `next-app`, replacing the dead GTM install and an earlier interim SPA-tracking-only fix attempt (superseded — the ported `Analytics.tsx` does the same job, properly consent-gated):
- `src/components/Analytics.tsx` — direct `gtag.js` via `next/script`, Consent Mode v2 defaults (denied until accepted), SPA pageview tracking via `usePathname`/`useSearchParams` (wrapped in `Suspense`), and a `ConsentBridge` that fires the deferred pageview once a visitor accepts.
- `src/components/CookieConsent.tsx` + `src/lib/consent.ts` — the real Accept/Reject banner (bottom-fixed, glass style), state persisted in `localStorage` (`pg_cookie_consent`, with legacy-key migration from an older `pg_consent` key), dispatching a `pg-consent-changed` event `Analytics.tsx` listens for.
- `src/components/CookiePreferences.tsx` — the "manage preferences" control embedded in the Privacy page's Cookies section (`#cookies`), showing live Accepted/Rejected/Not set status with Accept/Reject/Reopen-banner buttons.
- `src/components/CookiePreferencesLink.tsx` — plain button used by `Footer.tsx`'s new "Cookie Preferences" link (Compliance & Trust column) to reopen the banner from anywhere on the site.
- `app/layout.tsx` — removed the dead `GTM-TBJDVSX4` bootstrap script/noscript; mounted `<Analytics />`, `<CookieConsent />`, and `<Toaster />` (the last needed because `CookieConsent`/`CookiePreferences` use `useToast` for confirmation toasts — see the `toaster.tsx` bug below).
- `app/privacy/PrivacyContent.tsx` — replaced the placeholder cookie-banner line with the real "Your Choice" copy + `<CookiePreferences />`.
- `next-app/.env.local` — added `NEXT_PUBLIC_GA_ID=G-KQ1ERBR9PW` (gitignored, so this alone doesn't cover deployment — see outstanding steps).

**Bug found and fixed along the way:** `src/components/ui/toaster.tsx` was missing its `"use client"` directive — a latent, pre-existing bug that had never surfaced because nothing had ever actually mounted `<Toaster />` in `next-app` before this fix. Caused a hard 500 (`useState only works in Client Components`) the first time it was rendered from the (server) root layout. Fixed by adding the directive.

**Verified (dev, via Playwright):** fresh visit → Consent Mode v2 defaults all `denied` before `gtag.js` loads, initial `page_view` still recorded in a consent-denied state (correct, cookieless-ping behavior); clicking Accept → `localStorage` set to `accepted`, `consent update` (`granted`) pushed to `dataLayer`, banner dismisses, deferred `page_view` fires; clicking through `/` → `/solutions` (client-side `next/link` navigation, no reload) correctly pushed a new `page_view` with the right `page_path`; Privacy page's `CookiePreferences` control correctly shows live "Accepted" status and its "Reopen banner" button works; Footer's new "Cookie Preferences" link correctly reopens the banner from the homepage too. `tsc --noEmit` passes clean.

**Outstanding before this can ship:**
- [ ] `npm run build` not run yet — dev server was live at the time (known dev/build corruption gotcha in CLAUDE.md); run it once before merging.
- [ ] Add `NEXT_PUBLIC_GA_ID=G-KQ1ERBR9PW` to Vercel Project Settings (Production + Preview) and redeploy — `.env.local` alone only covers local dev (same "env vars are snapshotted per-deployment" gotcha as the Turnstile/Supabase vars elsewhere in this file).
- [ ] Decide what to do with the now-fully-confirmed-dead `GTM-TBJDVSX4` container — it has never had a tag published or drafted in its entire history (confirmed via GTM's own Version History and Workspace view), and this fix doesn't use it at all. Options: leave it registered but unused, or decommission it in the GTM dashboard.
- [ ] `aelhaddadmhe-alt/PurpleGuardRepl` (the source repo this was ported from) may hold other work that also never made it into this repo — worth a broader diff pass at some point, not just the analytics/consent files.

**Files involved:**
- `next-app/src/components/Analytics.tsx`, `CookieConsent.tsx`, `CookiePreferences.tsx`, `CookiePreferencesLink.tsx` (new, ported)
- `next-app/src/lib/consent.ts` (new, ported)
- `next-app/app/layout.tsx` (GTM removed, real components wired in)
- `next-app/src/components/Footer.tsx` (Cookie Preferences link added)
- `next-app/app/privacy/PrivacyContent.tsx` (placeholder copy replaced with real copy + control)
- `next-app/src/components/ui/toaster.tsx` (`"use client"` bug fix)
- `next-app/.env.local` (added `NEXT_PUBLIC_GA_ID`, not yet added to Vercel)
